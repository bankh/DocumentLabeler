...
if __name__ == '__main__':
  ...
  options = [
      ...
      CustomArgs(['--bs', '--batch_size'], default=2, type=int, 
                  target='train_data_loader;args;batch_size',
                  help='batch size (default: 2)'),
      # CustomArgs(['--ng', '--n_gpu'], default=2, type=int, target='n_gpu',
      #            help='num of gpu (default: 2)'),
      CustomArgs(['-dist', '--distributed'], default='false', type=str, 
                  target='distributed',
                  help='run distributed training. (true or false, default: true)'),
      CustomArgs(['--local_world_size'], default=1, type=int, 
                  target='local_world_size',
                  help='the number of processes running on each node, this is passed \
                        in explicitly and is typically either $1$ or the number of \
      ...
      CustomArgs(['--local_rank'], default=0, type=int, target='local_rank',
                  ...
                  process will be assigned a local rank ID in [0, local_world_size-1]. \
                  (default: 0)')
  ]
    
  # Option for debugging without the cli entry
  args.add_argument('--config', default='/mnt/data_drive/CSU_PhD/research/software/PICK-pytorch/config.json')
  args.add_argument('--device', default='0')
  # args.add_argument('--resume', default=None)
  args.add_argument('--resume', default=None)
