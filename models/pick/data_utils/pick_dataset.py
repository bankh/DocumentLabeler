...
class PICKDataset(Dataset):
    def __init__(self, files_name: str = None,
        ...
        # Check and print the existence of boxes_and_transcripts folder
        # print("Boxes and Transcripts folder:", self.boxes_and_transcripts_folder)
    ...
    def __getitem__(self, index):
        ...
        if boxes_and_transcripts_file.stat().st_size == 0 or image_file.stat().st_size == 0:
            if self.ignore_error:
                warnings.warn(f"Warning: Skipping empty file {boxes_and_transcripts_file.name} or {image_file.name}. Generating a new item.")
                new_item = random.randint(0, len(self) - 1)
                return self.__getitem__(new_item)
            else:
                # raise RuntimeError(f"Error: File {boxes_and_transcripts_file.name} or {image_file.name} is empty.")
                print(f"Error: File {boxes_and_transcripts_file.name} or {image_file.name} is empty.")
        ...
        try:
            if self.training:
                document = documents.Document(boxes_and_transcripts_file, 
                                              image_file, 
