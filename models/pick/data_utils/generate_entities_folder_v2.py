...
def process_file(input_file, output_folder, keys):
    ...
    with open(input_file, 'r') as tsvfile, open(output_file, 'w') as outfile:
        ...
        for row in tqdm(reader, desc=f'Processing {os.path.basename(input_file)}', total=sum(1 for _ in reader)):
...
if __name__ == '__main__':
    ...
    # Print the list of output files and log to a file
    with open(os.path.join(output_folder, 'output_files.txt'), 'w') as f:
        f.write('\n'.join(results))

    # Move the 'entities' folder to the parent folder of the '.tsv' folder
    parent_folder = os.path.dirname(os.path.dirname(args.input_folder))
    entities_folder = os.path.join(parent_folder, 'entities')
    os.replace(output_folder, entities_folder)

    print(f'\nMoved the "entities" folder to {entities_folder}')