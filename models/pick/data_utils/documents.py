...
MAX_BOXES_NUM = 200 # Set to a relatively lower value for large documents single documents handled separately.
MAX_TRANSCRIPT_LEN = 30  # limit max length text of every box
...
# class Document:
#     def __init__(self, boxes_and_transcripts_file: Path, 
#                  image_file: Path,
...
class Document:
    def __init__(self, boxes_and_transcripts_file: Optional[Path] = None,
                 ...
                 image_index=None,
                 max_boxes_num=MAX_BOXES_NUM,
                 direct_boxes_and_transcripts: Optional[List[Tuple[int, List[float], str, str]]] = None):
        '''
        An item returned by dataset.
        ...
        and entities_file must be set.
        :param image_index: image index, used to get image file name
        :param direct_boxes_and_transcripts: Directly provided boxes and transcripts data
        '''
        self.resized_image_size = resized_image_size
        self.training = training
        assert iob_tagging_type in ['box_level', 'document_level', 'box_and_within_box_level'], \
        ...
        self.image_filename = image_file.as_posix() if image_file else None
        self.max_boxes_num = max_boxes_num

        try:
            if direct_boxes_and_transcripts is not None:
                # Use the directly provided boxes and transcripts data
                boxes_and_transcripts_data = direct_boxes_and_transcripts
            else:
                # Read boxes, transcripts, and entity types of boxes in one document from the file
                if self.training:
                    boxes_and_transcripts_data = read_gt_file_with_box_entity_type(boxes_and_transcripts_file.as_posix())
                else:
                    boxes_and_transcripts_data = read_ocr_file_without_box_entity_type(boxes_and_transcripts_file.as_posix())
            ...
            if image_file:
                ...
                raise ValueError("Image file must be provided.")
        except Exception as e:
            raise IOError('Error occurs in image {}: {}'.format(image_file.stem, e.args))
        ...
        try:
            ...
            self.mask = RawField().preprocess(mask)
            self.boxes_num = RawField().preprocess(boxes_num)
            self.transcript_len = RawField().preprocess(transcript_len)  # Max transcript length of current document
            if self.training:
                self.iob_tags_label = IOBTagsField.preprocess(iob_tags_label)
            else:
                self.image_index = RawField().preprocess(image_index)

            print(f"Type of boxes_coordinate: {type(self.boxes_coordinate)}")
        except Exception as e:
            raise RuntimeError('Error occurs in image {}: {}'.format(boxes_and_transcripts_file.stem if boxes_and_transcripts_file else "direct input", e.args))
