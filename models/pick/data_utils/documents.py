...
MAX_BOXES_NUM = 100        # limit max number boxes of every documents
...
def print_text_segments(text_segments):
    texts, texts_len = text_segments

    for idx, (row, length) in enumerate(zip(texts, texts_len)):
    ...
    def __init__(self, boxes_and_transcripts_file: Path, 
        ...
        try:
            ...
            for i in range(boxes_num):
                ...
                resized_rect_output_i = cv2.minAreaRect(np.array(resized_box_i, 
                ...
                # formula (9)
                self.relation_features_between_ij_nodes(boxes_num, 
                                                        i, 
                                                        min_area_boxes, 
                                                        relation_features, 
                                                        transcript_i,
            ...
            relation_features = normalize_relation_features(relation_features, 
                                                            width=width, 
                                                            height=height)
            # print('Size relation features:',relation_features.shape)
            ...
            if self.training:
                ...
                if self.iob_tagging_type == 'box_level':
                    ...
                    iob_tags_label = text2iob_label_with_document_level_exactly_match(transcripts[:boxes_num], 
                                                                                      entities)
