...
# Delete the divided training set, validation set, and test set 
# folders, and recreate an empty folder
def isCreateOrDeleteFolder(path, 
                           flag):
...
def splitTrainVal(root, 
    ...
    # Divide the training set, validation set, and test set according 
...
if __name__ == "__main__":
    ...
    # 说明：可以根据自己的路径和需求调整参数，图像数据往往多人合作分批标注，
    # 每一批图像数据放在一个文件夹内用PPOCRLabel进行标注，
    # 如此会有多个标注好的图像文件夹汇总并划分训练集、验证集、测试集的需求
    
    # Function description: divide the training set, verification set and test set of detection 
    # and recognition respectively
    # Description: You can adjust the parameters according to your own path and needs. 
    # Image data is often marked by multiple people in batches. Each batch of image data is 
    # placed in a folder and marked with PPOCRLabel.
    # In this way, there will be multiple labeled image folders to summarize and divide the 
    # training set, verification set, and test set requirements
    parser = argparse.ArgumentParser()
    parser.add_argument("--trainValTestRatio",
                        type=str,
                        default="6:2:2",
                        help="ratio of trainset:valset:testset")
    parser.add_argument("--datasetRootPath",
                        type=str,
                        default="../train_data/",
                        help="path to the dataset marked by ppocrlabel,\
                              E.g, dataset folder named 1,2,3...")
    parser.add_argument("--detRootPath",
                        type=str,
                        default="../train_data/det",
                        help="the path where the divided detection dataset is placed")
    parser.add_argument("--recRootPath",
                        type=str,
                        default="../train_data/rec",
                        help="the path where the divided recognition dataset is placed")
    parser.add_argument("--detLabelFileName",
                        type=str,
                        default="Label.txt",
                        help="the name of the detection annotation file")
    parser.add_argument("--recLabelFileName",
                        type=str,
                        default="rec_gt.txt",
                        help="the name of the recognition annotation file")
    parser.add_argument("--recImageDirName",
                        type=str,
                        default="crop_img",
